...
bool CRedisRequest::setbit(const s8* key, u32 keyLen, u32 offset, bool val) {
...
bool CRedisRequest::getbit(const s8* key, u32 keyLen, u32 offset) {
    if (nullptr == key || 0 == keyLen) {
        return false;
    }
    const u32 cnt = 3;
...
bool CRedisRequest::bitcount(const s8* key, u32 keyLen, u32 min, u32 max) {
    if (nullptr == key || 0 == keyLen) {
        return false;
    }
    const u32 cnt = 4;
...
bool CRedisRequest::bitpos(const s8* key, u32 keyLen, bool val, u32 min, u32 max) {
    if (nullptr == key || 0 == keyLen) {
        return false;
    }
    const u32 cnt = 5;
...
bool CRedisRequest::bitop(const s8* key, u32 keyLen, u32 flag,
...
bool CRedisRequest::bitfiled(const s8* key, u32 keyLen,
    const s8** val, const u32* valLens, u32 count) {
    if (nullptr == key || 0 == keyLen || nullptr == val || 0 == valLens
        || 0 == count) {
        return false;
