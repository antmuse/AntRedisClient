...
CRedisResponse::CRedisResponse(CMemoryHub& it) :
    mUsed(0),
    mAllocated(0),
    mHub(it),
    mStatus(EDS_INIT),
...
CRedisResponse::~CRedisResponse() {
    clear();
    mHub.drop();
}

void CRedisResponse::clear() {
    if (EDS_INIT == mStatus) {
        return;
    }
    switch (mType) {
    ...
    case ERRT_ARRAY:
        if (mValue.mNodes) {
            for (u32 i = 0; i < mAllocated; ++i) {
                if (mValue.mNodes[i]) {
                    CRedisResponse* nd = reinterpret_cast<CRedisResponse*>(mValue.mNodes[i]);
                    nd->clear();
                    mHub.release(nd);
                    mHub.drop();
                }
    ...
    for (; str < end; ++str) {
        ...
        case EDS_ARRAY_SIZE:
            if (decodeSize(str[0])) {
                ...
                mValue.mNodes = reinterpret_cast<CRedisResponse**>
                    (mHub.allocateAndClear(sizeof(CRedisResponse*)*mAllocated));
                for (u32 i = 0; i < mAllocated; ++i) {
                    mValue.mNodes[i] = new
                    (reinterpret_cast<CRedisResponse*>(mHub.allocateAndClear(sizeof(CRedisResponse)))) CRedisResponse(mHub);
                }
            }
            break;
        ...
        case EDS_ARRAY:
        {
            CRedisResponse* nd = reinterpret_cast<CRedisResponse*>(mValue.mNodes[mUsed]);
...
void CRedisResponse::show(u32 level, u32 index)const {
    if (EDS_DONE != mStatus) {
