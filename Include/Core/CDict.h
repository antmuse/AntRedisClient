...
typedef void (AppHashDictScanCallback)(void* iUserData, const SDictNode* de);
typedef void (AppHashSlotScanCallback)(void* iUserData, SDictNode** bucketref);
...
public:
    ...
    * should be called while iterating. */
    ...
    CDict(const SDictFunctions& iType, void* iUserPointer = nullptr);
    ...
    * at a given time, it's just a few CDict properties xored together.
    * When an unsafe iterator is initialized, we get the CDict fingerprint, and check
    ...
    * performed forbidden operations against the dictionary while iterating. */
    ...
    void setKey(SDictNode* entry, const void* _key_) const {
    ...
    void setValue(SDictNode* entry, const void* _val_)const {
    ...
    * iteration calls.
    *
    * CDict's hash tables are always power of two in size, and they
    ...
    * See addRaw() for more information. */
    SDictNode* addOrFind(const void* key);
    ...
    */
    SDictNode* unlink(const void* key);

    SDictNode* find(const void* key);

    /**
    * @return a random entry from the hash table.
    * Useful to implement randomized algorithms */
    SDictNode* getRandomKey();
    ...
    * points to an array of SDictNode pointers. The array must have room for
    ...
    * at producing N elements. */
    u64 getRandomKeys(SDictNode** des, u64 count);

    u64 getHash(const void* key);

    /**
    * @brief Finds the SDictNode reference by using pointer and pre-calculated hash.
    * oldkey is a dead pointer and should not be accessed.
    * the hash value should be provided using getHash().
    * @return The reference to the SDictNode if found, or nullptr if not found. */
    SDictNode** findNodeRefByPtrAndHash(const void* oldptr, u64 hash);
    ...
    * to unlink(). It's safe to call this function with 'he' = nullptr. */
    void releaseNode(SDictNode* he);

    void releaseKey(SDictNode* entry)const {
        if(mCallers.mKeyDestructor) {
            mCallers.mKeyDestructor(mUserPointer, entry->mKey);
        }
    }

    void releaseValue(SDictNode* entry)const {
...
private:
    ...
    void* mUserPointer;
    SDictFunctions mCallers;
    CHashTable mHashTable[2];
    ...
    */
    SDictNode* genericRemove(const void* key, s32 nofree);


    /**
    * @brief Low level add or find:
    * This function adds the entry but instead of setting a value returns the
    * SDictNode structure to the user, that will make sure to fill the value
    * field as he wishes.
    *
    * This function is also directly exposed to the user API to be called
    * mainly in order to store non-pointers inside the hash value, example:
    *
    * entry = addRaw(CDict,mykey,nullptr);
    ...
    */
    SDictNode* addRaw(const void* key, SDictNode** existing);
    ...
    * index is always returned in the context of the second (new) hash table. */
    s64 getKeyIndex(const void* key, u64 hash, SDictNode** existing);
